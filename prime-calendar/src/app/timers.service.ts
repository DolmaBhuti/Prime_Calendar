...
export class TimersService {
  ...
  addTimer(timer: Timer): Observable<any> {
    console.log(
      `URL: ${environment.userAPIBase}/events/${timer.eventId}/timers/add`
    );
    return this.http.post<any>(
      `${environment.userAPIBase}/events/${timer.eventId}/timers/add`,
      timer
    );
  }
  ...
  deleteTimer(eventId: any, timerId: any): Observable<any> {
    return this.http.delete<any>(
      `${environment.userAPIBase}/events/${eventId}/timers/${timerId}`
    );
  }
}
